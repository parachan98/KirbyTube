...
<table width="100%" align="center" cellpadding="0" cellspacing="0" border="0">
	<tr valign="top">
			...
			<tr>
				...
				<div class="watchTitleBar">
					<table width="100%" cellpadding="0" cellspacing="0" border="0">
						<tr valign="top">
							<td><div class="watchTitle">Favorites // <span style="text-transform: capitalize;"><?php echo $_GET["user"]; ?></span></div></td>
 							<td align="right"> 
								<div style="font-weight: bold; color: #444; margin-right: 5px;">
								<?php echo "Videos " . $page . "-" . $stmt->rowCount() + $page - 1 . " of " . $favorite_videos;?>
								</div>
							</td>
						</tr>
					</table>
				...
				if($total_pages > 1) { 
					...
					<span style="<?= $page == $p ? 'color: #444; background-color: #FFFFFF;' : 'color: #CCC;'; ?> padding: 1px 4px 1px 4px; border: 1px solid #999; margin-right: 5px;"><?= $page == $p ? $p . '</span>' : '<a href="profile_favorites.php?user=' . $_GET["user"] . '&page=' . $p . '">' . $p . '</a></span>' ?>
				<?php }}?>
				</div>
				<!-- end paging -->
		...
		<div style="font-size: 14px; font-weight: bold; margin-bottom: 10px; color: #444;">&#187; Favorites</div>
